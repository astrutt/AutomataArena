...
async def handle_grid_hardware(node, nick: str, reply_target: str, action: str = None, args: list = None):
    ...
    if not action:
        ...
        async with node.db.async_session() as session:
            ...
            await node.send(f"{reply_method} {private_target} :{tag_msg(header, tags=['GEOINT'], location=gn.name)}")
            ...
            await node.send(f"{reply_method} {private_target} :{tag_msg(f'Chassis Slots: {slot_info}', tags=['GEOINT'])}")
            ...
            if meters:
                await node.send(f"{reply_method} {private_target} :{tag_msg(' | '.join(meters), tags=['GEOINT'])}")
            
            footer = format_text(f"Use '{node.prefix} grid hardware install <item>' to augment architecture.", C_YELLOW)
            await node.send(f"{reply_method} {private_target} :{tag_msg(footer, tags=['GEOINT'])}")
        return
    ...
    if action == "install":
        ...
        if result['success']:
            await node.send(f"{reply_method} {private_target} :{tag_msg(format_text(result['msg'], C_GREEN), tags=['SIGACT', nick])}")
            # Public narrative
            ...
            await node.send(f"{reply_method} {private_target} :{tag_msg(format_text(result['msg'], C_RED), tags=['SIGACT', nick])}")
        ...
        if result['success']:
            await node.send(f"{reply_method} {private_target} :{tag_msg(format_text(result['msg'], C_CYAN), tags=['SIGACT', nick])}")
        else:
            await node.send(f"{reply_method} {private_target} :{tag_msg(format_text(result['msg'], C_RED), tags=['SIGACT', nick])}")
