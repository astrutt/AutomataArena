...
async def handle_admin_command(node, admin_nick: str, verb: str, args: list, reply_target: str):
    ...
    msg_target, broadcast_chan, machine, action_cmd = await get_action_routing(node, admin_nick, reply_target)
    
    # Handle !a admin <subcommand>
    if verb == "admin":
        if not args:
            # Landing Page
            await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text('[ MAINFRAME ADMIN OVERRIDES ]', C_CYAN, True), tags=['SIGINT'], nick=admin_nick)}")
            ...
            await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(cmd_str, C_WHITE), tags=['SIGINT'], nick=admin_nick)}")
            return
    ...
    if verb == "version":
        # System Versions
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text('[ SYSTEM VERSION ARCHIVE ]', C_CYAN, True), tags=['SIGINT'], nick=admin_nick)}")
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text('Mainframe Core: v1.8.0-STABLE', C_WHITE), tags=['SIGINT'], nick=admin_nick)}")
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text('DB Orchestrator: v1.8.0 | Repositories: v1.8.0', C_GREEN), tags=['SIGINT'], nick=admin_nick)}")
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text('Command Router: v1.8.0 | AI Bot Client: v1.8.0', C_YELLOW), tags=['SIGINT'], nick=admin_nick)}")
    elif verb == "status":
        ...
        # 4. Multi-line Report
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text('[ MAINFRAME TELEMETRY ]', C_CYAN, True), tags=['SIGINT'], nick=admin_nick)}")
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(f'UPTIME: {uptime} | STATUS: {b_stat} | BOTS: {len(players)}', C_WHITE), tags=['SIGINT'], nick=admin_nick)}")
        
        grid_msg = f"GRID: {grid['claimed_nodes']}/{grid['total_nodes']} nodes ({grid['claimed_percent']:.1f}%) | MESH: {grid['total_power']:.0f}uP"
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(grid_msg, C_GREEN), tags=['SIGINT'], nick=admin_nick)}")
        
        econ_msg = f"ECON: {econ['total_credits']:.0f}c Total Liquidity | {econ['total_data_units']:.1f}u Total Data"
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(econ_msg, C_YELLOW), tags=['SIGINT'], nick=admin_nick)}")
        
        queue_msg = f"QUEUE: {len(node.match_queue)} in line | {len(node.ready_players)} ready to drop"
        await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(queue_msg, C_CYAN), tags=['SIGINT'], nick=admin_nick)}")
        ...
        if node.active_engine and node.active_engine.active:
            ...
            await node.send(f"{action_cmd} {reply_target} :[SYS] Match aborted.")
        else: await node.send(f"{action_cmd} {reply_target} :[SYS] No active battle.")
    elif verb == "battlestart":
        if node.active_engine and node.active_engine.active: await node.send(f"{action_cmd} {reply_target} :[SYS] Arena locked.")
        ...
        if len(full_args) >= 3 and full_args[0].lower() == "rename":
            ...
            await node.send(f"{action_cmd} {msg_target} :{tag_msg(feedback, tags=[tag], nick=admin_nick)}")
            if success:
            ...
            await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(f'Initiating procedural grid expansion ({count} nodes)...', C_YELLOW), tags=['SIGINT'], nick=admin_nick)}")
            ...
            await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(feedback, C_GREEN), tags=['SIGINT'], nick=admin_nick)}")
            ...
            if len(full_args) >= 2:
                ...
                await node.send(f"{action_cmd} {msg_target} :{tag_msg(format_text(f'Current Grid Nexus: {current_spawn}', C_CYAN), tags=['SIGINT'], nick=admin_nick)}")
        else:
            await node.send(f"{action_cmd} {msg_target} :[ERR] Syntax: {node.prefix} admin grid <rename|seed|spawn> [args]")
        ...
        if verb == "nickregister":
            ...
            if len(args) >= 2 and args[0].lower() == "rotate":
                try:
                    ...
                    await node.send(f"{action_cmd} {msg_target} :{tag_msg(msg, tags=['SIGINT'], nick=admin_nick)}")
                except Exception as e:
                    logger.error(f"Topic rotate error: {e}")
                    await node.send(f"{action_cmd} {msg_target} :[ERR] Syntax: {node.prefix} admin chantopic rotate <min>")
            else:
                await node.send(f"{action_cmd} {msg_target} :[ERR] Syntax: {node.prefix} admin chantopic rotate <min>")
    elif verb == "restart":
